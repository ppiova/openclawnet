...
public sealed class AgentProfileVaultIntegrationTests
{
    ...
    public async Task ResolveProfileFieldsAsync_WithDeletedSecret_ThrowsInvalidOperationException()
    {
        ...
        Assert.Contains("Failed to resolve vault reference for Endpoint", ex.Message);
        Assert.Contains("required secret was not found or could not be accessed", ex.Message);
    }
